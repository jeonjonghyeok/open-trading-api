...
> **최종 업데이트: 2026-04-17 (test_mode 진입 완화 + DRY 주문 홀딩 버그 수정 반영)**
...
| 항목 | 값 |
|------|-----|
| 매수 조건 (운용) | RSI < 35 AND MACD 히스토그램 음→양 전환 |
| 매수 조건 (테스트) | `test_mode: true` → RSI < 60 단일 조건 (MACD 불필요) |
| 매도 조건 (운용) | RSI > 70 OR MACD 양→음 OR 손절 -3% OR 익절 +5% |
| 매도 조건 (테스트) | 손절 -2% OR 익절 +2% (빠른 순환) |
| 종목 선별 | KOSPI 거래량 순위 상위 5종목 (실전 앱키 없으면 대형주 fallback) |
| 가격 범위 | 5,000 ~ 300,000원 |
| 포지션 | 최대 3종목 / 종목당 1주 / 시장가 |
| 모드 | demo (모의투자) |

> **현재 `test_mode: true`** — 테스트 기간 중 완화 조건 적용. 실전 전환 시 `config.json`에서 `test_mode: false`로 변경.
...
| 항목 | 값 |
|------|-----|
| 메인 페이지 | https://www.notion.so/344275c10f8e8044891cd8242c78e3ed |
| 전략 히스토리 DB | https://www.notion.so/cc3163894f234ca7873ef0f5e42925fe |
| 데이터소스 ID | `fe32674e-0d66-42fd-9037-087e6e7677d1` |
| 📋 체결내역 | https://www.notion.so/344275c10f8e81c5ba4dc47d1887a4b3 |
| 💼 보유포지션 | https://www.notion.so/344275c10f8e81da88dec2c515a12a3b |
| 📈 전략성과 | https://www.notion.so/344275c10f8e81238f78f3d53230970f |
| 📅 일일리포트 | https://www.notion.so/344275c10f8e819cae9fd84df96dfad7 |
| 뷰 | 📅 날짜별 (Table) / 🔄 상태별 (Board) |
| 기록 시점 | 매일 15:35 `run_review.py` 실행 후 자동 push |

**Notion 대시보드 구성 (4개 하위 페이지):**
- 📋 **체결내역** — KIS 일일 체결 기준. 시간·종목·구분·수량·가격·손익·전략
- 💼 **보유포지션** — 잔고 조회 기준. 보유 종목·수량·평균단가·평가손익 스냅샷
- 📈 **전략성과** — 누적 주문수·승률·손익비·MDD·파라미터 변경 이력
- 📅 **일일리포트** — logs/YYYY-MM-DD_review.json 기반 당일 요약 + AI 피드백

**전략 히스토리 DB 각 행 포함 내용:**
- 날짜·버전·상태 (파라미터유지·파라미터수정·전략변경)
- 전략 파라미터 전체 (변경 전→후 비교)
- AI 피드백 내용 및 변경 내역 테이블
...
- [x] `kis-strategy-skills/` 마켓플레이스 스캐폴딩
- [x] CLAUDE.md: 전략 스킬 마켓플레이스·`strategy-implementer` 스킬 선택 모델 추가
- [x] Notion 대시보드 4개 하위 페이지 생성 (체결내역·보유포지션·전략성과·일일리포트)
- [x] `ai_trading_explainer.html` 현행화 (Notion 대시보드 섹션 반영, 구글시트→Notion)
- [x] Notion 메인 페이지 재구성 (HTML 스타일 시각화)
- [x] `test_mode` 진입 완화 — `config.json` + `signals.py` 수정 (RSI < 60 단일 조건, ±2% 손익)
- [x] `executor.py` `get_holdings()` 버그 수정 — DRY 주문이 실제 보유로 인식되던 문제 해결 (`order_no != 'DRY'` 조건 추가)
- [x] 카카오(035720) 모의투자 매수 주문 정상 접수 확인 (주문번호 0000011524)

### 다음 세션 시작점

1. **실전 앱키 연동** — `kis_devlp.yaml`에 `my_app`, `my_sec` 추가 → 거래량순위 API 활성화
2. **market_regime.py** — KOSPI 장 국면 분류 (추세/횡보/급등락)로 전략 맥락화
3. **risk_guard.py** — 주문 전 한도 검증 (종목당 최대 비중, 당일 최대 손실 한도)
4. **전략 다변화** — `strategy_vol_surge.py` 거래량 급등 전략 추가
5. **실 성과 확인** — `run_cycle.py` 실행 후 Notion 대시보드 (체결내역·전략성과) 누적 확인
6. **notion_reporter.py 확장** — 체결내역·보유포지션·전략성과·일일리포트 4개 하위 페이지 자동 업데이트 구현
